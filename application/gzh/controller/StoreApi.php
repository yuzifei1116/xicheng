...
class StoreApi extends AuthController
{
     ...
     * 获取分类
     * @author: gyz
     * @Time: 2020/4/29 14:59
     */
    public function get_category_one_two()
    {
        $list = StoreCategory::getProductCategory();
    ...
    {
        $id = input('id/d',0);
        if(!$id || !($storeInfo = StoreProduct::getValidProduct($id))) return JsonService::failjson('商品不存在或已下架');
        ...
        $count = setView($this->uid,$id,$storeInfo['cate_id'],'viwe');
        $data['storeInfo'] = StoreProduct::setLevelPrice($storeInfo,$this->uid,true);
        $data['storeInfo']['browse'] = $count;
...
//        $data['mer_id'] = StoreProduct::where('id',$storeInfo['id'])->value('mer_id');
    ...
    public function collect_product($productId,$category = 'product')
    {
        ...
        if(!$res) return JsonService::failjson(StoreProductRelation::getErrorInfo());
        else return JsonService::successfuljson();
    }
    ...
    public function collect_product_all($productId = '',$category = 'product'){
        ...
        $res = StoreProductRelation::productRelationAll($productIdS,$this->uid,'collect',$category);
        if(!$res) return JsonService::failjson(StoreProductRelation::getErrorInfo());
        else return JsonService::successfuljson('收藏成功');
    }
     ...
     * @return \think\response\Json
     */
    public function get_user_collect_product($page = 1,$limit = 8)
    {
        return JsonService::successfuljson(StoreProductRelation::getUserCollectProduct($this->uid,$page,$limit));
    }
    /**
     * 获取收藏产品删除
     * @param int $first
    ...
    {
        if($pid){
            $list = StoreProductRelation::where('uid',$this->uid)->where('product_id',$pid)->delete();
            return JsonService::successfuljson($list);
        }else
            return JsonService::failjson('缺少参数');
    }
    ...
    public function goods_search()
    {
        ...
        return JsonService::successfuljson(StoreProduct::getSearchStorePage($keyword,$this->uid));
    }
...
//            ['news',0],
        ...
        return JsonService::successfuljson(StoreProduct::getProductList($data,$this->uid));
    }
    ...
    public function get_pid_cate(){
        ...
        if(Cache::has('one_pid_cate_list'))
            return JsonService::successfuljson(Cache::get('one_pid_cate_list'));
        else{
    ...
    public function get_id_cate(Request $request){
        ...
        return JsonService::successfuljson($dataCate);
    }
    ...
    {
        return JsonService::successfuljson(['userCollect'=>StoreProductRelation::isProductRelation($product_id,$this->uid,'collect')]);
    }
    /**
     * 获取产品评论
     * @param int $productId
    ...
    public function get_product_reply($productId = 0){
        ...
        return JsonService::successfuljson(['replyCount'=>$replyCount,'reply'=>$reply]);
    }
    ...
    public function get_order_product($unique = ''){
        if(!$unique || !StoreOrderCartInfo::be(['unique'=>$unique]) || !($cartInfo = StoreOrderCartInfo::where('unique',$unique)->find())) return JsonService::failjson('评价产品不存在!');
        return JsonService::successfuljson($cartInfo);
    }
    ...
    {
        if(!$productId) return JsonService::failjson('缺少参数');
        return JsonService::successfuljson(StoreProductReply::productReplyCount($productId));
    }
    ...
    public function product_attr_detail($productId = '')
    {
        ...
        return JsonService::successfuljson(compact('productAttr','productValue'));
...
//        if(empty($productInfo)) return JsonService::failjson('参数错误');
...
//            else return JsonService::failjson('没有查看权限');
//        }
//        $posterPath = createPoster($productInfo);
//        return JsonService::successfuljson($posterPath);
    }
    ...
    public function product_promotion_code($id = 0){
        if(!$id) return JsonService::failjson('参数错误ID不存在');
        $count = StoreProduct::validWhere()->count();
        if(!$count) return JsonService::failjson('参数错误');
        $path = makePathToUrl('routine/product/',4);
        if($path == '') return JsonService::failjson('生成上传目录失败,请检查权限!');
        $codePath = $path.$id.'_'.$this->uid.'_product.jpg';
        ...
        return JsonService::successfuljson($domain.$codePath);
    }
    ...
    public function get_routine_hot_search(){
        ...
        return JsonService::successfuljson($routineHotSearch);
    }
     ...
     */
    public function get_store_visit()
    {
        $list = StoreVisit::getUserStoreVisit($this->uid);
        return JsonService::successfuljson($list);
    }
}