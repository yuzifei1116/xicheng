...
class CouponsApi extends AuthController
{
    ...
    {
        $types = input('get.types/d',0);
        $page = input('get.page/d',1);
        $limit = input('get.limit/d',10);
        switch ($types){
            case 0:case '':
                $list= StoreCouponUser::getUserAllCoupon($this->userInfo['uid'],$page,$limit);
                break;
            case 1:
                $list=StoreCouponUser::getUserValidCoupon($this->userInfo['uid'],$page,$limit);
                break;
            case 2:
                $list=StoreCouponUser::getUserAlreadyUsedCoupon($this->userInfo['uid'],$page,$limit);
                break;
            default:
                $list=StoreCouponUser::getUserBeOverdueCoupon($this->userInfo['uid'],$page,$limit);
                break;
        }
    ...
    {
        $res = StoreCouponUser::getUserAllCoupon($this->userInfo['uid']);
...
//        }else{
//            return JsonService::failjson(StoreCouponIssue::getErrorInfo('领取失败!'));
//        }
//    }
    ...
    public function get_issue_coupon_list()
    {
        ...
        $page = input('page/d',1);
        $mer_id = input('mer_id/d',0);
//        $res = StoreCouponIssue::getIssueCouponList($this->uid,$limit,$page);
        $res = StoreCoupon::getCanGetCoupon($this->uid,$page,$limit,$mer_id);
        return JsonService::successfuljson($res);
