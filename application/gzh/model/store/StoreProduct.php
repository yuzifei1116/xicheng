...
//    }
     ...
     * @Time: 2020/6/8 17:57
     */
    public static function getCheckMerId($all = true)
    {
        $mer = MerchantList::getCheckMerId();
        if ($all) array_push($mer,0);
        $res = $mer;
...
//        return self::where('is_del',0)->where('is_show',1)->whereIn('mer_id',self::getCheckMerId());
        return self::where('is_del',0)->where('is_show',1)->where('status',1)->where('mer_status',1);
    }
...
//        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'sales DESC' : 'sales ASC';//真实销量
        ...
        $model=$model->order($baseOrder.'sort DESC,mer_sort desc, add_time DESC');
...
//            ->where('mer_id',0)
    ...
    {
        if($unique){
            $res = false !== StoreProductAttrValue::incProductAttrStock($productId,$unique,$num);
     ...
     */
    public static function getMerchantProduct($id,$price_order,$sales_order,$page,$limit,$type)
    {
        $model = self::where('mer_id',$id)
            ->where('status',1)
            ->where('is_del',0)
            ->where('mer_status',1);
        $price_order = $price_order == 'asc' ? 'price asc' : 'price desc';
        $sales_order = $sales_order == 'asc' ? 'sales asc' : 'sales desc';
        if ($type == 1){ //0综合排序 1价格 2 销量
            $model = $model->order($price_order.',mer_sort desc');
        }elseif($type == 2){
            $model = $model->order($sales_order.',mer_sort desc');
        }else{
            $model = $model->order('mer_sort desc');
        }
        $product = $model->page($page,$limit)->select();
