    ...
    public static function createPink($order){
        ...
        if($order['pink_id']){//拼团存在
            ...
            foreach ($order['cartInfo'] as $v){
                ...
                $cominfo = self::getCominfo($v['combination_id']);

                $pink['com_type'] = $cominfo['com_type'];
                $pink['min_people'] = $cominfo['min_people'];
...
//                $pink['stop_time'] = time()+86400;//结束时间
                $pink['stop_time'] = $cominfo['stop_time'];//结束时间
                $pink['add_time'] = time();//开团时间

                if($cominfo['com_type'] == 1){
                    $map14 = [
                        'cid' => $v['combination_id'],
                        'k_id' => 0
                    ];
                    $kinfo = self::where($map14)->find();
                    $kinfo = !empty($kinfo) ? $kinfo->toArray() : [];
                    if (!empty($kinfo)) $pink['k_id'] = $kinfo['id'];
                    ...
                    if ($haspeople >= $cominfo['min_people']){
                        $res = StorePink::where('id|k_id',$order['pink_id'])->update(['status'=>2]);
                        $pink['status'] = 2;//最后一人拼团后立刻完成
                    }
                }
    ...
    public static function getCominfo($combination_id)
    {
        $res = StoreCombination::where('id',$combination_id)->find();
//        $res = self::where('id',$combination_id)->find();
        return $res;
    }
    /**
     * 获取超过结束时间的团
     * gyz
     */
    public static function getOverEndTime1($com_type=1,$cid = 0)
    {

        $map = [
            'com_type' => $com_type,
            'status' => 1,//大团只退进行中的团，完成的团不退
            'stop_time' => array('<',time()),
        ];

        if (is_array($com_type)) $map['com_type'] = array('in', $com_type);

        if ($cid) $map['cid'] = $cid;
        $res = self::where($map)->select();
    ...
    public static function changeStatus($id,$status)
    {
        $data = ['status'=>$status];
        $map = ['id'=>$id];
        if (is_array($id)) $map = ['id'=>array('in',$id)];
        $res = self::where($map)->update($data);
        return $res;
    }
    /**
     * 修改退款状态
     ...
     * gyz
     */
    public static function changeIsRefund($id,$is_refund)
    {
