...
class UserExtract extends ModelBasic
{
    ...
    public static function userExtract($userInfo,$data){
        ...
        if($userInfo['now_money_spread'] < $data['extract_price'])
        ...
        $balance = bcsub($userInfo['now_money_spread'],$data['extract_price']);
        $insertData = [
            'uid'=>$userInfo['uid'],
        ...
        $res3 = UserBill::expend('佣金提现',$userInfo['uid'],'now_money_spread','extract',$data['extract_price'],$res1['id'],$balance,$mark);
        $res = $res2 && $res3;
        WechatTemplateService::sendTemplate(
            ...
            Url::build('gzh/My/balance',[],true,true)
            );
        if($res)
            return true;
        else
