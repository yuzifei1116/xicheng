...
use app\ebapi\model\user\UserNoticeSee;
...
//        }
     ...
     * @date:2020/5/9 15:58
     */
    public function user_spread_order($page = 1,$limit = 10)
    {
        $list = StoreOrder::getUserSpreadOder($this->uid,$page,$limit);
...
//            HookService::listen('store_product_order_reply',$group,$cartInfo,false,StoreProductBehavior::class);
    ...
    */
    ...
    public function get_user_bill_spread_list($page=1,$limit=8,$type=0)
    {
        return JsonService::successful(UserBill::getUserBillSpreadList($this->uid,$page,$limit,$type));
    }
    ...
    {
        $count = 0;
        if($type == 3) $count = UserBill::getRecordCount($this->uid, 'now_money_spread', 'brokerage');
