...
//        $extractPrice = (float)bcsub($brokerage,$extractTotalPrice,2);//减去已提现金额
        $extractPrice = (float)bcsub($userInfo['now_money_spread'],self::userExtractTotalPrice($userInfo['uid'],0),2);//减去提现申请中的金额
        ...
        $balance = bcsub($userInfo['now_money_spread'],$data['money'],2);
        ...
        try{
            ...
            $res2 = User::edit(['now_money_spread'=>$balance],$userInfo['uid'],'uid');
            $res3 = UserBill::expend('佣金提现',$userInfo['uid'],'now_money_spread','extract',$data['money'],$res1['id'],$balance,$mark);
            $res = $res2 && $res3;
            if($res){
                self::commitTrans();
                //发送模板消息
