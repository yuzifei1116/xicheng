...
//        dump($config);die;
    ...
    public static function merPay($merinfo)
    {
        $testparm = [
            'partner_trade_no' => $merinfo['order_num'], // 商户订单号，需保持唯一性(只能是字母或者数字，不能包含有符号)
            'openid' => $merinfo['openid'],
            'check_name' => 'NO_CHECK', // NO_CHECK：不校验真实姓名, FORCE_CHECK：强校验真实姓名
            're_user_name' => '', // 如果 check_name 设置为FORCE_CHECK，则必填用户真实姓名
            'amount' => (int)$merinfo['extract_price']*100, // 企业付款金额，单位为分
            'desc' => '佣金提现', // 企
            'spbill_create_ip' => '47.105.38.177'
        ];
//        dump($testparm);die;
//        $testparm = [
//            'partner_trade_no' => '1233455', // 商户订单号，需保持唯一性(只能是字母或者数字，不能包含有符号)
//            'openid' => 'o-noR5bDyeSHtthOh2BTm5Jy1jfk',
//            'check_name' => 'NO_CHECK', // NO_CHECK：不校验真实姓名, FORCE_CHECK：强校验真实姓名
//            're_user_name' => '郭月振', // 如果 check_name 设置为FORCE_CHECK，则必填用户真实姓名
//            'amount' => 100, // 企业付款金额，单位为分
//            'desc' => '测试', // 企
//            'spbill_create_ip' => '47.105.38.169'
//        ];
//        return self::merchantPayService()->send($testparm);
        $rs = self::merchantPayService()->send($testparm);
        return $rs->toArray();
//        dump($rs->toArray());
    }
