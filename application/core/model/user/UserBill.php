...
class UserBill extends ModelBasic
{
    ...
    public static function income($uid,$title,$department,$type,$number,$oid)
    {
        $pm = 1;
        return self::set(compact('uid','title','department','type','number','pm','oid'));
    }

    public static function expend($uid,$title,$department,$type,$number,$oid)
    {
        $pm = 0;
        return self::set(compact('uid','title','department','type','number','pm','oid'));
    }
    ...
    {
        return self::where('uid',$uid)->where('category','now_money_spread')->where('type','brokerage')->where('pm',1)
    ...
    public static function getUserBillSpreadList($uid,$page,$limit,$type)
    {
        $model=self::where('uid',$uid)
            ->where('category','now_money_spread')
            ->order('add_time desc')
            ->field(['FROM_UNIXTIME(add_time,"%Y-%m") as time','group_concat(id SEPARATOR ",") ids'])->group('time');
        switch ((int)$type){
            case 3:
     ...
     * @date:2020/5/9 11:39
     */
    public static function getNowMoneySpreadList($uid,$page,$limit)
    {
        $model=self::where('uid',$uid)
            ->where('category','now_money_spread')
            ->order('add_time desc')
     ...
     * @return array
     * @author: gyz
     * @Time: 2020/5/22 10:24
     */
    public static function getSpreadBilllist($uid, $page, $limit)
    {
        $model=self::where('uid',$uid)
            ->where('category','now_money_spread')
            ->order('add_time desc')
...
//        }
    ...
    public static function getUserBill($where)
    {
        $data = self::alias('b')->join('user u','b.uid=u.uid','left')
            ->field('b.*,u.nickname')
        ...
        foreach ($data as $k=>&$v){
            if ($v['department'] == 'activity'){
                $v['department'] = '活动';
            }else if($v['department'] == 'menu'){
                $v['department'] = '点餐';
            }else if($v['department'] == 'stay'){
                $v['department'] = '住宿';
            }else{
                $v['department'] = '其它';
            }
        ...
        $count = self::where('type','wechat')->count();
