...
class UserSign extends ModelBasic
{
    ...
    public static function setSignData($uid,$title='',$number=0,$balance=0)
    {
        ...
        $res1 = self::set(compact('uid','title','number','balance','add_time'));
        $res2 = UserBill::income($title,$uid,'integral','sign',$number,0,$balance,$title);
        return $res1&&$res2;
//        return self::set(compact('uid','title','number','balance','add_time')) && UserBill::income($title,$uid,'integral','sign',$number,0,$balance,$title);
     ...
     * @return mixed
     * @author: gyz
     * @Time: 2020/5/20 17:32
     */
    public static function getSignListWithTime($uid,$start_time,$end_time)
    {
...
//            $res1 = self::setSignData($uid,'用户累计签到第'.(self::getSignSumDay($uid)+1).'天',$sign_num,$user->integral);
        ...
        $sign_num = SystemConfigService::get('sx_sign_int');

        $res1 = self::setSignData($uid,'用户累计签到第'.(self::getSignSumDay($uid)+1).'天',$sign_num,$user->integral+$sign_num);
