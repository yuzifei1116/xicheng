            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="is_send">
                        ...
                        时间：{{d.send_time}}
                        {{# }else{ }}
                        状态：<span style="color:red;">未发送</span> <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event='send'>全员发送</a>
                        {{# } }}
                    </script>
                    <script type="text/html" id="act">
                        {{# if(d.is_send==0){ }}
                            <a class="layui-btn layui-btn-xs" href="javascript:;"  onclick="$eb.createModalFrame('编辑','{:Url('edit')}?id={{d.id}}')"><i class="fa fa-paste"></i> 编辑</a>
                        {{# } }}
                            <a class="layui-btn layui-btn-xs layui-btn-danger" href="javascript:;" lay-event='warning'><i class="fa fa-warning"></i> 删除</a>
                ...
                $eb.$swal('delete',function(){
                    $eb.axios.get(url).then(function(res){
                        if(res.status == 200 && res.data.code == 200) {
                            ...
                            obj.update({send_time:'{:date("Y-m-d H:i:s",time())}'});
                            obj.update({is_send:1});
                        }else
