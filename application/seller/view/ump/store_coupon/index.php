                ...
                <div class="table-responsive" style="overflow:visible">
                    <table class="table table-striped  table-bordered">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                {if condition="$vo['coupon_type']==1"}
                                <span>代金券</span>
                                {/if}
                                {if condition="$vo['coupon_type']==2"}
                                <span>折扣券</span>
                                {/if}
                            </td>
                            <td class="text-center">
                                ...
                                {/if}
                                {if condition="$vo['coupon_type']==2"}
                                {$vo.coupon_discount}折
                                {/if}
                            </td>
                            <td class="text-center">
                                {if condition="$vo.is_limit==1"}
                                {$vo.coupon_num}
                                {/if}
                                {if condition="$vo.is_limit==0"}
                                不限量
                                {/if}
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                {if condition="$vo.time_type==1"}
                                {$vo.coupon_long_time}天
                                {/if}
                                {if condition="$vo.time_type==2"}
                                {:date('Y-m-d',$vo['coupon_start_time'])} - {:date('Y-m-d',$vo['coupon_end_time'])}
                                {/if}
                                ...
                                <div class="input-group-btn js-group-btn">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a href="javascript:void(0);" onclick="$eb.createModalFrame(this.innerText+' [{$vo.title}] 优惠劵','{:Url('edit',['id'=>$vo['id']])}')">
