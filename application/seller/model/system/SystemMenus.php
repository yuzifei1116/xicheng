...
class SystemMenus extends ModelBasic
{
    ...
    {
        $menusList = self::where('is_show','1')->where('module','seller')->where('access','1')->order('sort DESC,id asc')->select();
    ...
    public static function tidyMenuTier($sellerFilter = false,$menusList,$pid = 0,$navList = [])
    {
        static $allAuth = null;
        static $sellerAuth = null;
        if($allAuth === null) $allAuth = $sellerFilter == true ? SystemRole::getAllAuth() : [];//所有的菜单
//        if($sellerAuth === null) $sellerAuth = $sellerFilter == true ? SystemAdmin::activeAdminAuthOrFail() : [];//当前登录用户的菜单
...
//                if($pid != 0 && $sellerFilter && in_array($authName,$allAuth) && !in_array($authName,$sellerAuth)) continue;
                $menu['child'] = self::tidyMenuTier($sellerFilter,$menusList,$menu['id']);
