...
class SystemNotice extends ModelBasic
{
    ...
    public static function sellerMessage($notice_type,$seller_id,$link_id,array $table_data = [])
    {
        $table_data = json_encode($table_data);
        $add_time = time();
        return self::systemNoticeAdminDb()->insert(compact('notice_type','seller_id','link_id','table_data','add_time'));
    }
    ...
    public static function noticeMessage($noticeType,$linkId,array $tableData = [])
    {
        ...
        $sellerIds = array_merge(array_map(function($v){
            return $v['id'];
        },SystemAdmin::getTopAdmin('id')->toArray())?:[],self::typeByAdminList($noticeType,'push_seller')->push_seller?:[]);
        $sellerIds = array_unique(array_filter($sellerIds));
        if(!count($sellerIds)) return self::setErrorInfo('没有有效的通知用户!');
        foreach ($sellerIds as $id){
            self::sellerMessage($noticeType,$id,$linkId,$tableData);
        }
        return true;
    }

    public static function getAdminNoticeTotal($sellerId)
    {
        $list = self::alias('A')->join('__SYSTEM_NOTICE_ADMIN__ B','B.notice_type = A.type')
            ->where('A.status',1)->where('B.is_visit',0)->where('B.is_click',0)->where('B.seller_id',$sellerId)
    ...
    public static function getAdminNotice($sellerId)
    {
        $list = self::alias('A')->join('__SYSTEM_NOTICE_ADMIN__ B','B.notice_type = A.type')
            ->where('A.status',1)->where('B.is_visit',0)->where('B.is_click',0)->where('B.seller_id',$sellerId)
        ...
        if(count($noticeTypeList))
            self::systemNoticeAdminDb()->where('notice_type','IN',$noticeTypeList)->where('seller_id',$sellerId)
