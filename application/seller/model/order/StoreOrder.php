...
class StoreOrder extends ModelBasic
{
    ...
    public static function orderCount($mer_id){
        $data['wz']=self::statusByWhere(0,new self())->where('mer_id',$mer_id)->count();
        $data['wf']=self::statusByWhere(1,new self())->where('mer_id',$mer_id)->count();
        $data['ds']=self::statusByWhere(2,new self())->where('mer_id',$mer_id)->count();
        $data['dp']=self::statusByWhere(3,new self())->where('mer_id',$mer_id)->count();
        $data['jy']=self::statusByWhere(4,new self())->where('mer_id',$mer_id)->count();
        $data['tk']=self::statusByWhere(-1,new self())->where('mer_id',$mer_id)->count();
        $data['yt']=self::statusByWhere(-2,new self())->where('mer_id',$mer_id)->count();
        $data['general']=self::where(['pink_id'=>0,'combination_id'=>0,'seckill_id'=>0,'bargain_id'=>0])->where('mer_id',$mer_id)->count();
        $data['pink']=self::where('pink_id|combination_id','>',0)->where('mer_id',$mer_id)->count();
        $data['seckill']=self::where('seckill_id','>',0)->where('mer_id',$mer_id)->count();
        $data['bargain']=self::where('bargain_id','>',0)->where('mer_id',$mer_id)->count();
        return $data;
    }

    public static function OrderList($where,$mer_id){
        $model = self::getOrderWhere($where,self::alias('a')->join('user r','r.uid=a.uid','LEFT'),'a.','r')->where('mer_id',$mer_id)->field('a.*,r.nickname');
        if($where['order']!=''){
...
HTML;
        ...
        $count=self::getOrderWhere($where,self::alias('a')->join('user r','r.uid=a.uid','LEFT'),'a.','r')->where('mer_id',$mer_id)->count();
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
    ...
    public static function getBadge($where,$mer_id){
        $price=self::getOrderPrice($where,$mer_id);
        return [
            [
                'name'=>'订单数量',
                'field'=>'件',
     ...
     * @return array
     */
    public static function getOrderPrice($where,$mer_id){
        ...
        $sumNumber =self::getOrderWhere($where,$model)->where('is_del',0)->where('paid',1)->where('mer_id',$mer_id)->field([
        ...
        $list=self::getOrderWhere($where,$model)->where('is_del',0)->where('refund_status','<>',2)->where('paid',1)->where('mer_id',$mer_id)->group('pay_type')->field(['sum(pay_price) as sum_pay_price','pay_type'])->select();
        foreach ($list as $v){
