...
use app\seller\model\setting\Merchant;
use app\seller\model\setting\SystemAdmin;
...
//        if($where['access'] !== '') $model = $model->where('access',$where['access']);
        ...
        return self::page($model,function($item){
            if(!$item['mer_id']) $item['seller_name'] = '总后台管理员---》'.SystemAdmin::where('id',$item['seller_id'])->value('real_name');
            else $item['seller_name'] = Merchant::where('id',$item['mer_id'])->value('mer_name').'---》'.MerchantAdmin::where('id',$item['seller_id'])->value('real_name');
...
//        exit();
    ...
    public static function merchantPage($where = array()){
        ...
        $model = $model
            ->where('status',1)
            ->where('hide',0)
            ->where('seller_id',$where['seller_id'])
