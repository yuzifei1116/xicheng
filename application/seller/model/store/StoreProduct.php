...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
     ...
     *
     */
    public static function ProductList($where,$mer_id){
        $model=self::getModelObject($where)->where('mer_id',$mer_id)->field(['p.*','sum(pav.stock) as vstock']);
        ...
        $count=self::getModelObject($where)->where('mer_id',$mer_id)->count();
     ...
     * @Time: 2020/5/9 17:01
     */
    public static function getAllByCategory($cate_ids,$mer_id = 0){
        $model = new self;
        $where = [
            'is_show'=>1,
            'is_del' => 0,
...
//        }
     ...
     */
    public static function getProductCount($mer_id)
    {
