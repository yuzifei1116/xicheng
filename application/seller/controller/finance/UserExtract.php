...
use app\seller\model\user\UserExtract as UserExtractModel;
...
//        dump($extract->toArray());die;
        $extract = $extract->toArray();
        $flag = true;
        if ($extract['extract_type'] == 'weixin'){
            //z走微信打款流程
            $openid = WechatUser::uidToRoutineOpenid($extract['uid']);
            $merinfo = [
                'order_num' => $id,
                'openid' => $openid,
...
//            dump($merinfo);
//            die;
            $res_mer = MiniProgramService::merPay($merinfo);
            if ($res_mer['return_code'] == 'SUCCESS' && $res_mer['result_code'] == 'SUCCESS'){
                //无
            }else{
                $flag = false;
            }
...
//            die;
        ...
        if($flag){
            ...
            if($res){
                ...
                return JsonService::fail('提现成功，保存操作失败!');
            }
        }else{
            return JsonService::fail('提现操作失败!');
        }
    }
}