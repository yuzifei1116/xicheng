...
use think\Url;
use app\seller\model\ump\StoreSeckillAttr;
use app\seller\model\ump\StoreSeckillAttrResult;
use app\seller\model\ump\StoreSeckill as StoreSeckillModel;
...
class StoreSeckill extends AuthController
{
    ...
    {
        $this->assign('countSeckill',StoreSeckillModel::getSeckillCount($this->sellerId));
        $this->assign('seckillId',StoreSeckillModel::getSeckillIdAll($this->sellerId));
        return $this->fetch();
    }
    public function save_excel(){
        ...
        StoreSeckillModel::SaveExcel($where,$this->sellerId);
    }
    /**
     * 异步获取砍价数据
     */
    public function get_seckill_list(){
        ...
        $seckillList = StoreSeckillModel::systemPage($where,$this->sellerId);
        ...
        return JsonService::successlayui(['count'=>$seckillList['list']['total'],'data'=>$data]);
    }

    public function get_seckill_id(){
        return JsonService::successlayui(StoreSeckillModel::getSeckillIdAll($this->sellerId));
    }
    /**
     * 添加秒杀产品
     * @return form-builder
    ...
    public function create()
    {
        ...
        $f[] = Form::frameImageOne('image','产品主图片(305*305px)',Url::build('seller/widget.images/index',array('fodder'=>'image')))->icon('image');
        $f[] = Form::frameImages('images','产品轮播图(640*640px)',Url::build('seller/widget.images/index',array('fodder'=>'images')))->maxLength(5)->icon('images');
    ...
    public function save(Request $request,$id = 0)
    {
        ...
        if(!$data['product_id']) return JsonService::fail('产品ID不能为空');
//        var_dump($this->request->post());
        ...
        if($data['price'] == '' || $data['price'] < 0) return JsonService::fail('请输入产品秒杀售价');
        ...
        if($data['num']<1) return JsonService::fail('请输入单次秒杀个数');
        if($id){
    ...
    public function seckill($id){
        ...
        $f[] = Form::dateTimeRange('section_time','活动时间');
        $f[] = Form::frameImageOne('image','产品主图片(305*305px)',Url::build('seller/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image');
        $f[] = Form::frameImages('images','产品轮播图(640*640px)',Url::build('seller/widget.images/index',array('fodder'=>'images')),json_decode($product->getData('slider_image')))->maxLength(5)->icon('images');
        $f[] = Form::number('price','秒杀价')->min(0)->col(12);
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::frameImageOne('image','产品主图片(305*305px)',Url::build('seller/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image');
        $f[] = Form::frameImages('images','产品轮播图(640*640px)',Url::build('seller/widget.images/index',array('fodder'=>'images')),json_decode($product->getData('images')))->maxLength(5)->icon('images');
    ...
    public function delete($id)
    {
        if(!$id) return $this->failed('数据不存在');
        $product = StoreSeckillModel::get($id);
        if(!$product) return JsonService::failjson('数据不存在!');
        if ($product['mer_id'] != $this->sellerId) return JsonService::failjson('数据错误');
        $data['is_del'] = 1;
        if(!StoreSeckillModel::edit($data,$id))
            return JsonService::fail(StoreSeckillModel::getErrorInfo('删除失败,请稍候再试!'));
        else
    ...
    public function edit_content($id){
        ...
        if(!$seckill) return JsonService::fail('数据不存在!');
    ...
    public function change_field(Request $request,$id,$field){
        ...
        $seckill = StoreSeckillModel::get($id);
        if(!$seckill) return JsonService::fail('数据不存在!');
        if ($seckill['mer_id'] != $this->sellerId) return JsonService::fail('数据错误');
