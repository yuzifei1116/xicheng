...
use think\Url;
use app\seller\model\ump\StorePresaleAttr;
use app\seller\model\ump\StorePresaleAttrResult;
use app\seller\model\ump\StorePresale as StorePresaleModel;
...
//        $this->assign('presaleId',StorePresaleModel::getPresaleIdAll());
    ...
    public function save_excel(){
        ...
        StorePresaleModel::SaveExcel($where,$this->sellerId);
    }
    /**
     * 异步获取砍价数据
     */
    public function get_presale_list(){
        ...
        $presaleList = StorePresaleModel::systemPage($where,$this->sellerId);
...
//        }
        return JsonService::successlayui(['count'=>$presaleList['list']['total'],'data'=>$data]);
    }

    public function get_presale_id(){
        return JsonService::successlayui(StorePresaleModel::getPresaleIdAll($this->sellerId));
    }
    /**
     * 添加预售产品
     * @return form-builder
...
//        var_dump($this->request->post());
        ...
        if(empty($data['start_time'])) return JsonService::fail('请选预售时间');
...
//        if(count($data['weikuan_time'])<1) return JsonService::fail('请选尾款支付时间');
        ...
        if($data['price'] == '' || $data['price'] < 0) return JsonService::fail('请输入产品定金');
        if($data['pre_price'] == '' || $data['pre_price'] < 0) return JsonService::fail('请输入产品预售售价');
...
//        $f[] = Form::radio('is_hot','热门推荐',1)->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]])->col(12);
    ...
    public function delete($id)
    {
        ...
        if(!StorePresaleModel::edit($data,$id))
            return JsonService::fail(StorePresaleModel::getErrorInfo('删除失败,请稍候再试!'));
        else
    ...
    public function edit_content($id){
        ...
        if(!$presale) return JsonService::fail('数据不存在!');
        if($presale['mer_id'] != $this->sellerId) return JsonService::fail('数据错误!');
        $this->assign([
