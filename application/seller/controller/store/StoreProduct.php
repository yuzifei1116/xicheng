...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        //出售中产品
        $onsale =  ProductModel::where(['is_show'=>1,'is_del'=>0])->where('mer_id',$this->sellerId)->count();
        //待上架产品
        $forsale =  ProductModel::where(['is_show'=>0,'is_del'=>0])->where('mer_id',$this->sellerId)->count();
        //仓库中产品
        $warehouse =  ProductModel::where(['is_del'=>0])->where('mer_id',$this->sellerId)->count();
        //已经售馨产品
        $outofstock = ProductModel::getModelObject()->where(ProductModel::setData(4))->where('mer_id',$this->sellerId)->count();
        //警戒库存
        $policeforce =ProductModel::getModelObject()->where(ProductModel::setData(5))->where('mer_id',$this->sellerId)->count();
        //回收站
        $recycle =  ProductModel::where(['is_del'=>1])->where('mer_id',$this->sellerId)->count();
    ...
    public function product_ist(){
        ...
        return JsonService::successlayui(ProductModel::ProductList($where,$mer_id));
    }
    /**
     * 设置单个产品上架|下架
     *
    ...
    public function set_show($is_show='',$id=''){
        ...
        if (empty($product)) return JsonService::failjson('数据错误');
...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            Form::frameImageOne('image','产品主图片(305*305px)',Url::build('seller/widget.images/index',array('fodder'=>'image')))->icon('image')->width('100%')->height('500px'),
            Form::frameImages('slider_image','产品轮播图(640*640px)',Url::build('seller/widget.images/index',array('fodder'=>'slider_image')))->maxLength(5)->icon('images')->width('100%')->height('500px')->spin(0),
...
//            Form::radio('is_new','首发新品',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),
    ...
    public function save(Request $request)
    {
        ...
        $merchant = MerchantList::get($this->sellerId); //查询改商铺的商铺等级
        $number = MerchantGrade::getNumber($merchant['grade']); //可发布商品数量
        $count = ProductModel::getProductCount($this->sellerId); //获取该商铺发布的商品的数量
        if ($count >= $number) return JsonService::failjson('您发布的商品已达到最大数量,请提升商铺等级');
        ...
        if (MerchantList::getProductExamine($this->sellerId)){ //商品是否需要审核 0不需要 1需要
            $data['status'] = 0;
        }else{
            $data['status'] = 1;
        }
    ...
    public function edit_content($id){
        ...
        return $this->fetch('public/edit_content_seller');
    }
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::frameImageOne('image','产品主图片(305*305px)',Url::build('seller/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image')->width('100%'),
            Form::frameImages('slider_image','产品轮播图(640*640px)',Url::build('seller/widget.images/index',array('fodder'=>'slider_image')),json_decode($product->getData('slider_image'),1) ? : [])->maxLength(5)->icon('images')->width('100%')->height('500px'),
...
//            Form::number('give_integral','赠送积分',$product->getData('give_integral'))->min(0)->precision(0)->col(8),
            ...
            Form::number('mer_sort','排序',$product->getData('mer_sort'))->col(8),
