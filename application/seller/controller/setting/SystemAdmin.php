...
use think\Url;
use app\seller\model\system\SystemAdmin as AdminModel;
...
class SystemAdmin extends AuthController
{
    ...
    public function index()
    {
        ...
        $where = Util::getMore([
            ['name',''],
            ['roles',''],
            ['level',bcadd($seller->level,1,0)]
        ],$this->request);
        $this->assign('where',$where);
        $this->assign('role',SystemRole::getRole(bcadd($seller->level,1,0)));
    ...
    public function create()
    {
        ...
        $f[] = Form::select('roles','管理员身份')->setOptions(function ()use($seller){
                    $list = SystemRole::getRole(bcadd($seller->level,1,0));
                    $options = [];
    ...
    public function save(Request $request)
    {
        ...
        unset($data['conf_pwd']);
        $data['level'] = $this->sellerInfo['level'] + 1;
        AdminModel::set($data);
    ...
    public function edit($id)
    {
        ...
        $seller = AdminModel::get($id);
        if(!$seller) return Json::fail('数据不存在!');
        $f = array();
        $f[] = Form::input('account','管理员账号',$seller->account);
        $f[] = Form::input('pwd','管理员密码')->type('password');
        $f[] = Form::input('conf_pwd','确认密码')->type('password');
        $f[] = Form::input('real_name','管理员姓名',$seller->real_name);
        $f[] = Form::select('roles','管理员身份',explode(',',$seller->roles))->setOptions(function ()use($seller){
            $list = SystemRole::getRole($seller->level);
            $options = [];
            foreach ($list as $id=>$roleName){
                $options[] = ['label'=>$roleName,'value'=>$id];
            }
    ...
    public function sellerInfo(){
        $sellerInfo = $this->sellerInfo;//获取当前登录的管理员
...
//            if ($data['pwd'] == '') unset($data['pwd']);
            if($data['pwd'] != ''){
                $pwd = md5($data['pwd']);
                if($sellerInfo['pwd'] != $pwd) return Json::fail('原始密码错误');
            }
            if($data['new_pwd'] != ''){
            ...
            AdminModel::edit($data,$sellerInfo['id']);
