...
use app\seller\model\system\SystemNotice as NoticeModel;
...
class SystemNotice extends AuthController
{
    public function index(){
        $this->assign(NoticeModel::page(function($notice){
            $notice->push_seller_name = !empty($notice->push_seller) ? implode(',',SystemAdmin::where('id','IN',$notice->push_seller)->column('real_name')) : '';
        }));
        return $this->fetch();
    }
     ...
     public function create()
     {
         ...
         $f[] = Form::frameInputOne('icon','图标',Url::build('seller/widget.widgets/icon',array('fodder'=>'icon')))->icon('ionic');
         $f[] = Form::input('template','通知模板');
         $f[] = Form::input('table_title','通知数据')->type('textarea')->placeholder('数据1-key1,数据2-key2');
         $f[] = Form::select('push_seller','通知管理员')->setOptions(function(){
             $list = SystemAdmin::getOrdAdmin('real_name,id')?:[];
             $options = [];
             foreach ($list as $seller){
                 $options[] = ['label'=>$seller['real_name'],'value'=>$seller['id']];
             }
             return $options;
         })->multiple(1);
     ...
     public function save(Request $request)
     {
         $data = UtilService::postMore([
             ...
             ['push_seller', []], ['status', 0]
         ], $request);
         $data['push_seller'] = array_unique(array_filter($data['push_seller']));
     ...
     */
     public function edit($id)
     {
         ...
         $f[] = Form::frameInputOne('icon','图标',Url::build('seller/widget.widgets/icon',array('fodder'=>'icon')),$data->icon)->icon('ionic');
         ...
         $f[] = Form::select('push_seller','通知管理员',$data->push_seller)->setOptions(function(){
             $list = SystemAdmin::getOrdAdmin('real_name,id')?:[];
             $options = [];
             foreach ($list as $seller){
                 $options[] = ['label'=>$seller['real_name'],'value'=>$seller['id']];
