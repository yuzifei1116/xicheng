...
<!--    </div>-->
    <div class="layui-form-item" >
        ...
        <div class="layui-input-block imginput">
            <img src="{$data['img']}" class="layui-upload-img" id="demo1" width="100px" height=100px">
            <input type="hidden" data-value="{$data['img']}" value="{$data['img']}" name="img" id="img">
        </div>
    </div>
...
<script type="text/javascript">
    layui.use(['form','laydate','upload','colorpicker'], function(){
        ...
        var uploadInst = upload.render({
            elem: '#test1'
            ,url: "{:url('admin/ad.routine_ad/upload')}" //改成您自己的上传接口
            ,before: function(obj){
                //预读本地文件示例，不支持ie8
        ...
        form.on('submit(demo1)', function(data){
            ...
            $.ajax({
                type:"post",
                url:"{:url('admin/ad.routine_ad/update')}",
                data:data.field,
                dataType:"json",
