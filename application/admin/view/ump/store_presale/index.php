    ...
    <div class="layui-row layui-col-space15"  id="app">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">预售产品搜索</div>
                <div class="layui-card-body">
<!--                    <div class="alert alert-success alert-dismissable">-->
<!--                        <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>-->
<!--                        目前拥有{ $countPresale}个预售产品-->
<!--                    </div>-->
                        ...
                        <div class="layui-form-item">
                            <div class="layui-inline">
                                ...
                                <div class="layui-input-inline">
                                    <input type="text" name="title" lay-verify="store_name" style="width: 100%" autocomplete="off" placeholder="请输入产品名称" class="layui-input">
                                </div>
                            </div>
<!--                            <div class="layui-inline">-->
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="barDemo">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame('{{d.title}}-编辑','{:Url('create')}?id={{d.id}}')"><i class="layui-icon layui-icon-edit"></i> 编辑</a>
                            </li>
                            <li>
...
//    setTime();
    function setTime() {
        setTimeout(function () {
            $eb.axios.get("{:Url('get_presale_id')}").then(function(res){
...
<script>
    layList.form.render();
    layList.tableList('presaleList',"{:Url('get_presale_list')}",function () {
        return [
...
//            {field: 'ot_price', title: '原价',width:'6%'},
            {field: 'price', title: '定金',width:'6%'},
            {field: 'pre_price', title: '预售价',width:'6%'},
            {field: 'stock', title: '库存',width:'6%'},
            {field: 'start_name', title: '预售状态',width:'8%',toolbar:"#statusCn"},
            {field: 'stop_time', title: '预售结束时间', width: '13%'},
            {field: 'wk_stop_time', title: '尾款结束时间', width: '13%'},
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'delstor':
                var url=layList.U({c:'ump.store_presale',a:'delete',q:{id:data.id}});
    ...
    layList.search('export',function(where){
        location.href=layList.U({c:'ump.store_presale',a:'save_excel',q:{status:where.status,store_name:where.store_name}});
    })
    layList.switch('status',function (odj,value,name) {
        if (odj.elem.checked == true) {
            layList.baseGet(layList.Url({
                c: 'ump.store_presale',
                a: 'set_presale_status',
                p: {status: 1, id: value}
            }), function (res) {
                layList.msg(res.msg);
            });
