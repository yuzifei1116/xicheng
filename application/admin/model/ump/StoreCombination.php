...
class StoreCombination extends ModelBasic
{
   ...
   /**
    ...
    public static function ProfityTop10($where){
        $classs=['layui-bg-red','layui-bg-orange','layui-bg-green','layui-bg-blue','layui-bg-cyan'];
        $model = StoreOrder::alias('a')->join('__STORE_COMBINATION__ b','b.id = a.combination_id')->where('a.paid',1);
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.combination_id')->order('profity desc')->limit(10)
    ...
    public static function getBargainRefundList($where = array()){
        ...
        $list = self::getModelTime($where,$model,'a.add_time')->where('a.refund_status','NEQ',0)->group('a.combination_id')->order('count desc')->page((int)$where['page'],(int)$where['limit'])
            ->field(['count(a.combination_id) as count','b.title as store_name','sum(b.price) as sum_price'])->select();
        if(count($list)) $list=$list->toArray();
        return $list;
    }
...
//        $model = $model->join('StoreProduct p','p.id=s.product_id');
        if($where['com_type'] != '')  $model = $model->where('s.com_type',$where['com_type']);
        if($where['status'] != '')  $model = $model->where('s.status',$where['status']);
        if($where['title'] != '') $model = $model->where('s.id','LIKE',"%$where[title]%");
        $model = $model->page(bcmul($where['page'],$where['limit'],0),$where['limit']);
        $model = $model->order('s.id desc');
        $model = $model->where('mer_id',0)->where('s.is_del',0);
        return self::page($model,function($item){
//            $item['title'] = StoreProduct::where('id',$item['product_id'])->value('title');
//            $item['ot_price'] = StoreProduct::where('id',$item['product_id'])->value('ot_price');
            if($item['status']){
                ...
                if ($item['is_over'] == 1) $item['start_name'] = '活动已结束!';
    ...
    public static function SaveExcel($where){
        ...
        $list = $model->order('id desc')->where('mer_id',0)->where('is_del',0)->select();
        ...
        PHPExcelService::setExcelHeader(['编号','产品名称','活动简介','原价','秒杀价','库存','秒杀状态','结束时间','状态'])
    ...
    public static function getCombinationIdAll(){
        return self::where('is_del',0)->where('mer_id',0)->column('id','id');
    }
    ...
    public static function getCombinationCount(){
        return self::where('is_del',0)->where('mer_id',0)->count();
    }
}