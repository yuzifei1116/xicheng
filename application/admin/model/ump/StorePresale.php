...
class StorePresale extends ModelBasic
{
   ...
   /**
    ...
    public static function getMaxList($where){
        ...
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.presale_id')->order('p_count desc')->limit(10)
            ->field(['count(a.presale_id) as p_count','b.title as store_name','sum(b.price) as sum_price'])->select();
    ...
    public static function getBargainRefundList($where = array()){
        ...
        $list = self::getModelTime($where,$model,'a.add_time')->where('a.refund_status','NEQ',0)->group('a.presale_id')->order('count desc')->page((int)$where['page'],(int)$where['limit'])
            ->field(['count(a.presale_id) as count','b.title as store_name','sum(b.price) as sum_price'])->select();
        if(count($list)) $list=$list->toArray();
        return $list;
    }
...
//            $item['ot_price'] = StoreProduct::where('id',$item['product_id'])->value('ot_price');
     ...
     * @return array
     */
    public static function getPresaleIdAll(){
     ...
     * @return int|string
     */
    public static function getPresaleCount(){
