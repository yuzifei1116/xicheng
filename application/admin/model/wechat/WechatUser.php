...
//        self::setWechatUserOrder();//设置 一级推荐人 二级推荐人 一级推荐人订单 二级推荐人订单 佣金
        ...
        if($isall==false) {
            ...
            if ($status == 1) {
                $uids = User::where(['is_promoter' => 1])->column('uid');
                $model = $model->alias('w')->join('User u','w.uid=u.uid')->where('w.uid', 'in', implode(',', $uids));
            }else{
                $model = $model->alias('w')->join('User u','w.uid=u.uid');
            }
        }
//        $model = $model->where('openid','NOT NULL');
        if($where['nickname'] !== '') $model = $model->where('w.nickname','LIKE',"%$where[nickname]%");
        if($where['data'] !== ''){
            list($startTime,$endTime) = explode(' - ',$where['data']);
            $model = $model->where('w.add_time','>',strtotime($startTime));
            $model = $model->where('w.add_time','<',strtotime($endTime));
        }
        if(isset($where['tagid_list']) && $where['tagid_list'] !== ''){
            $tagid_list = explode(',',$where['tagid_list']);
            foreach ($tagid_list as $v){
                $model = $model->where('w.tagid_list','LIKE',"%$v%");
            }
        }
        if(isset($where['groupid']) && $where['groupid'] !== '-1' ) $model = $model->where('w.groupid',"$where[groupid]");
        if(isset($where['sex']) && $where['sex'] !== '' ) $model = $model->where('w.sex',"$where[sex]");
        if(isset($where['subscribe']) && $where['subscribe'] !== '' ) $model = $model->where('w.subscribe',"$where[subscribe]");
        ...
        if(isset($where['now_money_spread']) && $where['now_money_spread'] != '') $model = $model->order($where['now_money_spread']);
        $model = $model->field('w.*,u.*')->order('w.uid desc');
        if(isset($where['export']) && $where['export'] == 1){
            $list = $model->select()->toArray();
            $export = [];
            foreach ($list as $index=>$item){
