...
//        exit();
    ...
    {
        $data=self::setWhere($where,'a')
            ->join('ArticleCategory c','a.cid=c.id','left')
            ->join('ArticleContent n','a.id=n.nid','left')
            ->field('a.*,c.title as cate,n.content')
            ->where('a.status',1)
            ->page((int)$where['page'],(int)$where['limit'])
            ->select();

        $data=count($data) ? $data->toArray() : [];
        $count=self::setWhere($where,'a')->join('ArticleCategory c','a.cid=c.id','left')->join('ArticleContent n','a.id=n.nid','left')->count();
    ...
    public static function setWhere($where,$alias='',$model=null)
    {
        ...
        if(isset($where['title']) && $where['title']!='') $model=$model->where("{$alias}title",'like',"%$where[title]%");
        if(isset($where['cid']) && $where['cid'] != 0) $model=$model->where("{$alias}cid",$where['cid']);
        return $model;
    }
}