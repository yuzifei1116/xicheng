...
class StoreOrder extends ModelBasic
{
    ...
    public static function orderCount(){
        $data['wz']=self::statusByWhere(0,new self())->where('mer_id',0)->count();
        $data['wf']=self::statusByWhere(1,new self())->where('mer_id',0)->count();
        $data['ds']=self::statusByWhere(2,new self())->where('mer_id',0)->count();
        $data['dp']=self::statusByWhere(3,new self())->where('mer_id',0)->count();
        $data['jy']=self::statusByWhere(4,new self())->where('mer_id',0)->count();
        $data['tk']=self::statusByWhere(-1,new self())->where('mer_id',0)->count();
        $data['yt']=self::statusByWhere(-2,new self())->where('mer_id',0)->count();
        $data['general']=self::where(['pink_id'=>0,'combination_id'=>0,'seckill_id'=>0,'bargain_id'=>0])->where('mer_id',0)->count();
        $data['pink']=self::where('pink_id|combination_id','>',0)->where('mer_id',0)->count();
        $data['seckill']=self::where('seckill_id','>',0)->where('mer_id',0)->count();
        $data['bargain']=self::where('bargain_id','>',0)->where('mer_id',0)->count();
        return $data;
    }
    public static function orderMerchantCount(){
        $data['wz']=self::statusByWhere(0,new self())->where('mer_id','>',0)->count();
        $data['wf']=self::statusByWhere(1,new self())->where('mer_id','>',0)->count();
        $data['ds']=self::statusByWhere(2,new self())->where('mer_id','>',0)->count();
        $data['dp']=self::statusByWhere(3,new self())->where('mer_id','>',0)->count();
    ...
    public static function OrderList($where){
        $model = self::getOrderWhere($where,self::alias('a')->join('user r','r.uid=a.uid','LEFT'),'a.','r')->where('mer_id',0)->field('a.*,r.nickname');
        if($where['order']!=''){
...
//            $_info = Db::name('store_order_cart_info')->where('oid',$item['id'])->field('cart_info')->select();
            if (empty($item['order_id_father'])){
                $_info = Db::name('store_order_cart_info')->where('oid',$item['id'])->field('cart_info')->select();
            }else{
                $cids = json_decode($item['cart_id'],1);
                $_info = Db::name('store_order_cart_info')->whereIn('cart_id',$cids)->field('cart_info')->select();
//                dump($item['cart_id']);
//                echo Db::getLastSql();
//                dump($_info);die;
            }
...
HTML;
            ...
            //预售的单独判断
            if ($item['presale_id']>0){
                $item['pay_type_name']=$item['pay_type_name'].'定金';
                if ($item['paid']==1 && $item['presale_paid'] == 1) {
                    $presale_type = $item['presale_pay_type'] == 'weixin' ? '微信支付尾款' : '余额支付尾款';
                    $item['pay_type_name'] = $item['pay_type_name'] . "<br/>" . $presale_type;
                }
            }
        ...
        $count=self::getOrderWhere($where,self::alias('a')->join('user r','r.uid=a.uid','LEFT'),'a.','r')->where('mer_id',0)->count();
        return compact('count','data');
    }
    public static function MerchantOrderList($where){
...
HTML;
    ...
    public static function SaveExcel($list){
        ...
        PHPExcelService::setExcelHeader(['订单号','支付方式','商品总数','商品总价','邮费','支付金额','退款金额','用户备注','管理员备注','收货人姓名','收货人电话','收货人地址','商品信息','支付状态'])
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
    ...
    public static function getMerchantBadge($where){
        $price=self::getMerchantOrderPrice($where);
        return [
            [
                'name'=>'订单数量',
                'field'=>'件',
    ...
    public static function getOrderPrice($where){
        ...
        $sumNumber =self::getOrderWhere($where,$model)->where('is_del',0)->where('paid',1)->where('mer_id',0)->field([
        ...
        $list=self::getOrderWhere($where,$model)->where('is_del',0)->where('refund_status','<>',2)->where('paid',1)->where('mer_id',0)->group('pay_type')->field(['sum(pay_price) as sum_pay_price','pay_type'])->select();
        foreach ($list as $v){
     ...
     * @return array
     */
    public static function getMerchantOrderPrice($where){
        $where['is_del'] = 0;//删除订单不统计
        $model = new self;
        $price = array();
        $price['pay_price'] = 0;//支付金额
        ...
        $price['deduction_price'] = 0;//抵扣金额
        $price['total_num'] = 0; //商品总数
        $sumNumber =self::getOrderWhere($where,$model)->where('is_del',0)->where('paid',1)->field([
            'sum(total_num) as sum_total_num',
            'sum(pay_price) as sum_pay_price',
            'sum(refund_price) as sum_refund_price',
            'sum(use_integral) as sum_use_integral',
        ...
        $list=self::getOrderWhere($where,$model)->where('is_del',0)->where('refund_status','<>',2)->where('paid',1)->group('pay_type')->field(['sum(pay_price) as sum_pay_price','pay_type'])->select();
        foreach ($list as $v){
            if ($v['pay_type'] == 'weixin'){
                $price['pay_price_wx'] = $v['sum_pay_price'];
            }elseif($v['pay_type'] == 'yue'){
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function orderTakeAfter($order)
    {
        if($order['is_channel']){//小程序
            RoutineTemplate::sendOut('OREDER_TAKEVER',$order['uid'],[
                'keyword1'=>$order['order_id'],
                'keyword2'=>self::getDb('store_cart')->alias('a')->join('__STORE_PRODUCT__ P','a.product_id=p.id')->where('a.id','in',$order['cart_id'])->value('p.store_name'),
    ...
    public static function getOrderStair($where)
    {
        $arr = User::where('spread_uid',$where['uid'])->where('status',1)->column('uid');
        $uid = implode(',',$arr);
        $data = self::alias('o')
            ->join('user r','r.uid=o.uid','LEFT')
            ->where('o.uid','in',$uid)
            ->where('o.paid',1)
            ->where('o.status',3)
            ->field('o.*,r.nickname')
...
HTML;
        ...
        ->where('o.uid','in',$uid)
        ->where('o.paid',1)
        ->where('o.status',3)->count();
        return compact('data','count');
    }

    public static function getOrderSecond($where)
    {
        $arr = User::where('spread_uid',$where['uid'])->where('status',1)->column('uid');
        if (empty($arr)){
            $data = [];
        ...
        $uid = User::where('spread_uid',implode(',',$arr))->where('status',1)->column('uid');
        $uid = implode(',',$uid);
        $data = self::alias('o')
            ->join('user r','r.uid=o.uid','LEFT')
            ->where('o.uid','in',$uid)
...
HTML;
    ...
    public static function getChatrOder()
    {
        $legdata=['金额'];
        $list=self::field('FROM_UNIXTIME(add_time,"%Y.%c") as un_time,SUM(total_price) as total_price')
            ->group('un_time')
            ->distinct(true)
            ->select();
        ...
        $count = 0;
        return compact('datetime','chatrList','legdata','badge');
    }
}