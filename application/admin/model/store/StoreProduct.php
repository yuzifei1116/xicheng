...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
     ...
     *
     */
    public static function ProductsList($where){
        $model=self::getModelObject($where)->field(['p.*','sum(pav.stock) as vstock','m.mer_name'])->order('p.is_hot desc,p.is_benefit desc,p.is_best desc,p.mer_sort desc,p.id desc');
    ...
    public static function getProductStatistics()
    {
        ...
        $data = self::where('is_del',0)
            ->field('id,store_name,price,sales')
            ->order('sales desc')
            ->select();
        $data = $data->isEmpty() ? [] : $data->toArray();
        $array['sum_count'] = 0; //总数量
        $array['sum_price'] = 0; //总金额
        $array['list'] = [];
        foreach ($data as $k=>$v){
            $array['sum_count']  += $v['sales'];
            $array['sum_price'] += bcmul($v['price'],$v['sales'],2);
        }
        foreach ($data as $k=>$v){
            $array['list'][$k]['p_count'] = $v['sales'];
            $array['list'][$k]['store_name'] = $v['store_name'];
            $array['list'][$k]['sum_price'] = bcmul($v['price'],$v['sales'],2);
            $array['list'][$k]['w'] = bcdiv($v['sales'],$array['sum_count'],2)*100;
            $array['list'][$k]['class'] = isset($classs[$k]) ? $classs[$k] : (isset($classs[$k-count($classs)]) ? $classs[$k-count($classs)]:'');
        }

        $total_count = StoreOrder::group('uid')->count(); //用户购买的次数
        $two_count = User::where('product_count','>',1)->count();
        $array['two_count'] = bcdiv($two_count,$total_count,2)*100;
        return $array;
    }
     ...
     * @Time: 2020/5/9 17:01
     */
    public static function getAllByCategory($cate_ids){
        $model = new self;
        $where = [
            'is_show'=>1,
            'is_del' => 0,
...
//        }
        ...
        $model->where($where)->whereIn('cate_id',$cate_ids)->order('id desc');
        return $model->page($model,$where,'',28);
    }

    public static function getProduct($str = '')
    {
        $list = self::where('id','in',$str)->column('id,image');
        return $list;
    }
}