...
//        if(isset($where['producr_id']) && $where['producr_id']!=0) $model=$model->where('product_id',$where['producr_id']);
    ...
    {
        $list=self::valiWhere($where,'a')->group('p.id')->join('__WECHAT_USER__ u','u.uid=a.uid')->join("__STORE_PRODUCT__ p",'a.product_id=p.id')->where('p.mer_id',0)->field(['p.id','p.image','p.store_name','p.price'])->page($where['page'],$where['limit'])->select();
    ...
    public static function getProductReplyList($where)
    {
        ...
        $count=self::valiWhere($where,'a')->join('__WECHAT_USER__ u','u.uid=a.uid')->join("__STORE_PRODUCT__ p",'a.product_id=p.id')->where('a.product_id',$where['producr_id'])->where('p.mer_id',0)->count();
