    ...
    */
    ...
    public static function setCommissionWhere($where){
        $models=self::setWherePage(self::alias('A'),$where,[],['A.nickname','A.uid'])
            ...
            ->group('A.uid')
            ->where(['B.category'=>'now_money_spread','B.type'=>'brokerage'])
            ->field(['sum(B.number) as sum_number','A.nickname','A.uid','A.now_money_spread']);
        if($where['order']==''){
        ...
        if($where['price_max']!='' && $where['price_min']!=''){
            $models=$models->where('now_money_spread','between',[$where['price_max'],$where['price_min']]);
        }
        return $models;
    }
    ...
    public static function getUserinfo($uid){
        $userinfo=self::where(['uid'=>$uid])->field(['nickname','spread_uid','now_money','now_money_spread','add_time'])->find()->toArray();
        $userinfo['number']=UserBill::where(['category'=>'now_money_spread','type'=>'brokerage'])->sum('number');
...
//            ['name'=>'ID','value'=>$uid],
            ...
            ['name'=>'佣金总收入','value'=>UserBill::where(['category'=>'now_money_spread','type'=>'brokerage','uid'=>$uid])->sum('number')],
    ...
    public static function getDistributionBadgeList($where){
        return [
            [
                'name'=>'分销商总佣金',
                'field'=>'元',
                'count'=>self::getModelTime($where,UserBill::where('category','now_money_spread')->where('type','brokerage'))->where('uid','in',function($query){
            ...
            [
                'name'=>'分销商总佣金余额',
                'field'=>'元',
                'count'=>self::getModelTime($where,self::where('status',1)->where('is_promoter',1))->sum('now_money_spread'),
                'background_color'=>'layui-bg-cyan',
                'col'=>3,
            ],
            [
            ...
            [
                'name'=>'本月分销商业务佣金',
                'field'=>'元',
                'count'=>self::getModelTime(['data'=>'month'],UserBill::where('category','now_money_spread')->where('type','brokerage'))
            ...
            [
                'name'=>'上月分销商业务佣金',
                'field'=>'元',
                'count'=>self::getOldDate(['data'=>'year'],UserBill::where('category','now_money_spread')->where('uid','in',function ($query){
                    $query->name('user')->where('status',1)->where('is_promoter',1)->whereOr('spread_uid','neq',0)->field('uid');
...
//        echo self::getLastSql();die;
    ...
    public static function getUserDistributionTop10List($limit){
        ...
        $commission=UserBill::alias('l')
            ...
            ->where(['l.status'=>'1','l.category'=>'now_money_spread','l.type'=>'brokerage','a.is_promoter'=>1])
            ->group('l.uid')
    ...
    public static function getSpreadList($uid,$page,$limit){
        $list=self::where(['spread_uid'=>$uid])->field(['uid','nickname','now_money_spread','integral','add_time'])
    ...
    public static function backOrderBrokerage($orderInfo)
    {
        ...
        //返佣之后余额
        $spNow_money = User::where(['uid'=>$userInfo['spread_uid']])->value('now_money_spread');
    ...
    {
        $data = self::where('spread_uid',$where['uid'])
            ->where('status',1)
            ->field('avatar,nickname,now_money_spread,add_time,uid')
            ->page((int)$where['page'],(int)$where['limit'])
            ->order('add_time DESC')
            ->select();
        ...
        foreach ($data as $key=>$value){
            ...
            $data[$key]['orderCount'] = StoreOrder::getOrderCount($value['uid'])?:0;
        }
        $count = self::where('spread_uid',$where['uid'])
     ...
     * gyz修改
     */
    public static function getSecond($where)
    {
        $arr = self::where('spread_uid',$where['uid'])->where('status',1)->column('uid');
        if (empty($arr)){
            $data = [];
            $count = 0;
            return compact('data','count');
        ...
        $uid = implode(',',$arr);
        $data = self::where('spread_uid','in',$uid)
            ->where('status',1)
            ->field('avatar,nickname,now_money_spread,add_time,uid,spread_uid')
            ->page((int)$where['page'],(int)$where['limit'])
            ->order('add_time DESC')
            ->select();
        $data = $data->isEmpty() ? [] : $data->toArray();
        foreach ($data as $key=>&$value){
            ...
            $value['orderCount'] = StoreOrder::getOrderCount($value['uid'])?:0;
            $value['spread_name'] = self::where('uid',$value['spread_uid'])->value('nickname');
            $value['spread_id'] = self::where('uid',$value['spread_uid'])->value('uid');
        }
        $count = self::where('spread_uid','in',$uid)->where('status',1)->count();
        return compact('data','count');
    }
}