...
//            ['system_add','pay_product','extract','pay_product_refund','system_sub','brokerage','recharge','user_recharge_refund'],
     ...
     */
    public static function getNowMoneySpread($where)
    {
        $data = self::where('uid',$where['uid'])->where('category','now_money_spread')
            ->field('mark,number,add_time')
            ->where('status',1)
            ->order('add_time DESC')
            ->page((int)$where['page'],(int)$where['limit'])
            ->select();
        ...
        $count = self::where('uid',$where['uid'])->where('category','now_money_spread')->count();
