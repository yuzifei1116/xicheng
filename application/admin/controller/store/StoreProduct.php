...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        //出售中产品
        $onsale =  ProductModel::where(['is_show'=>1,'is_del'=>0])->where('mer_id',0)->count();
        //待上架产品
        $forsale =  ProductModel::where(['is_show'=>0,'is_del'=>0])->where('mer_id',0)->count();
        //仓库中产品
        ...
        $field = [
            Form::select('cate_id','产品分类')->setOptions(function(){
                ...
                foreach ($list as $menu){
                    $menus[] = ['value'=>$menu['id'],'label'=>$menu['cate_name']];//,'disabled'=>$menu['pid']== 0];
                }
                return $menus;
            })->filterable(0)->multiple(0),
