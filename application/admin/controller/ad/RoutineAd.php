...
use think\Request;
use app\admin\model\ad\RoutineAd as RoutineAdModel;
use think\Url;
...
class RoutineAd extends AuthController
{
    ...
    public function ad_list(){
        return JsonService::successlayui(RoutineAdModel::AdList());
    }
    /**
     * 设置单个产品上架|下架
     *
    ...
    public function set_show($is_show='',$id=''){
        ...
        $res=RoutineAdModel::where(['id'=>$id])->update(['is_show'=>(int)$is_show]);
        if($res){
...
//        dump($data);die;
    ...
    public function update()
    {
        ...
        if (empty($data['title'])) return JsonService::fail('标题不能为空');
        if (empty($data['img'])) return JsonService::fail('请上传图片');
        $data['img'] = is_array($data['img']) ? $data['img'][0] : $data['img'];
//        dump($data);die;
        $id = $data['id'];
        unset($data['file']);
        unset($data['id']);
        $res = RoutineAdModel::edit($data,$id);
        if ($res){
    ...
    public function delete($id)
    {
        ...
        if(!RoutineAdModel::del($id))
            return Json::fail(RoutineAdModel::getErrorInfo('删除失败,请稍候再试!'));
        else
