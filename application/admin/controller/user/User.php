...
class User extends AuthController
{
    ...
    public function edit($uid)
    {
        ...
        $f[] = Form::input('nickname','用户姓名',$user->getData('nickname'))->disabled(1);
        $f[] = Form::radio('integration_status','修改积分',1)->options([['value'=>1,'label'=>'增加'],['value'=>2,'label'=>'减少']]);
        $f[] = Form::number('integration','积分')->min(0);
        $f[] = Form::radio('is_stay','住宿权限',$user->getData('is_stay'))->options([['value'=>1,'label'=>'开启'],['value'=>0,'label'=>'关闭']]);
        $f[] = Form::radio('is_activity','住宿权限',$user->getData('is_activity'))->options([['value'=>1,'label'=>'开启'],['value'=>0,'label'=>'关闭']]);
    ...
    public function update(Request $request, $uid)
    {
        ...
        if($data['integration_status'] && $data['integration']){//积分增加或者减少
            if($data['integration_status'] == 1){//增加
                ...
                $res1 = UserBillAdmin::income('系统增加积分',$user['uid'],'integral','system_add',$data['integration'],$this->adminId,$edit['integral'],'系统增加了'.floatval($data['integration']).'积分');

            }else if($data['integration_status'] == 2){//减少
                if($user['integral'] < $data['integration']) Json::fail('用户积分不足'.$data['integration']);
                $edit['integral'] = bcsub($user['integral'],$data['integration'],2);
                $res1 = UserBillAdmin::expend('系统减少积分',$user['uid'],'integral','system_sub',$data['integration'],$this->adminId,$edit['integral'],'系统扣除了'.floatval($data['integration']).'积分');
            }
        }else{
            $res1 = true;
        }
        $edit['status'] = $data['status'];
        $edit['is_stay'] = $data['is_stay'];
        $edit['is_activity'] = $data['is_activity'];
        $res2 = UserModel::edit($edit,$uid);
