 ...
 */
class MerchantOrder extends AuthController
{
    /**
     * @return mixed
     */
    public function index()
    {
        ...
        $this->assign([
            ...
            'orderCount'=>StoreOrderModel::orderMerchantCount(),
    ...
    public function getBadge(){
        ...
        return JsonService::successful(StoreOrderModel::getMerchantBadge($where));
    }
    /**
     * 获取订单列表
     * return json
    ...
    public function order_list(){
        ...
        return JsonService::successlayui(StoreOrderModel::MerchantOrderList($where));
    }
...
//            if($data['type'] == 1)  StorePink::setRefundPink($id);
    ...
    public function jie_suan($id)
    {
        ...
        $order = StoreOrderModel::get($id);
        if(!$order) return Json::fail('数据不存在!');
        if($order['is_mer_jiesuan'] == 1) return Json::fail('不能重复结算!');
        if($order['status'] != 3 || $order['paid'] != 1 || $order['refund_status'] != 0) return Json::fail('该订单还结算!');
        if ($order['mer_id'] <= 0) return Json::fail('该订单不能结算!');
        $timeout = 20; //保险时间
