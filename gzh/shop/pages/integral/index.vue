<template>
	<view class="container">
		<view class="carousel">
			<swiper indicator-dots  interval="3000">
		...
		<view class="introduce-section">
			...
			<view class="price-box">
				...
				<text class="price">{{storeInfo.integral}}积分</text>
		...
		<view class="page-bottom ">
			...
			<view class="action-btn-group">
				...
				<button type="primary" class=" action-btn no-border buy-now-btn" :data-id="2" @click="complete">立即购买</button>
			</view>
		</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			details(){
				let self = this;
				let path = '/gzh/shop_api/product_detail?id='+self.detid
				getApp().http(path, {}, function(result) {
					self.storeInfo = result
					
					let simg = self.storeInfo.slider_image[0];
			...
			complete(){
				...
				if(self.clickId == '1'){
					...
					console.log('数量',self.num,self.detid);
					
					uni.navigateTo({
						url: `/pages/integral/integralOrder?id=`+self.detid+'&num='+self.num
					})
