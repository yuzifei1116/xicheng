	...
	<view>
		...
		<view  class="address-section">
			<view class="order-content">
				<view class="cen">
					<view class="">
						{{cartInfo.user_name}} {{cartInfo.user_phone}}
					</view>
					<view class="dz">
		...
		<view class="goods-section">
			...
			<!-- 商品列表 -->
			<view class="g-item">
				<image :src="indexUrl(cartInfo.info.image)"></image>
				<view class="right">
					<text class="title clamp">{{cartInfo.info.store_name}}</text>
					<view class="price-box">
						<text class="price">￥{{cartInfo.info.integral}}</text>
						<text class="number">x {{cartInfo.total_num}}</text>
					</view>
				</view>
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">支付积分:</text>
				<text class="cell-tip">{{cartInfo.total_integral}}</text>
			</view>
		...
		<view class="footer acea-row-reverse" v-if="cartInfo.status == 1">
			<view>
				<button class="submit" form-type="submit" @click="orderCon()">确认收货</button>
			</view>
		</view>
		
		<view class="footer acea-row-reverse" v-if="cartInfo.status == 2">
			<view>
...
<script>
	...
	export default {
		...
		methods: {
			confirm_order() {
				let self = this;
				let path = '/gzh/shop_api/order_detail'
				let data = {
					id: self.id
				}
				getApp().http(path, data, function(res) {
					...
					console.log('水电费水电费',self.cartInfo);
				})
			},
			...
			orderCon(){
				...
				uni.showModal({
					...
					success: (md) => {
						console.log('sssss')
						if(md.confirm == true){
							let path = '/gzh/shop_api/user_take_order'
							let data = {
								id: self.id
							}
							getApp().http(path, data, function(res) {
			...
			delOrder(){
				...
				uni.showModal({
					...
					success: (md) => {
						...
						if(md.confirm == true){
							let self = this;
							let path = '/gzh/shop_api/order_del'
							let data = {
								id: self.id
							}
							getApp().http(path, data, function(res) {
