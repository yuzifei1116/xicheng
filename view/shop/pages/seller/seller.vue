				...
				>
					...
					<view class="acea-row onr" >
						...
						<view class='text' @click="navClick(item)">
							<view class='line1'>店铺：{{item.mer_name}}</view>
							<view class='dz1'>地址：{{item.province}}{{item.city}}{{item.county}}{{item.address}}</view>
							<view class='dz1'>简介：{{item.info}}</view>
						</view>
						<view class="aces-space-between fot">
							<button class="dianp" hover-class="none" @click="phone(item)">
								联系客服
							</button>
								
							<!-- <view class="dianp">联系客服</view> -->
							<view class="dianp dianp1" @click="clickD(item)">进入店铺</view>
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			phone(item){
				...
				let customer_service = item.customer_service
				uni.showModal({
					title: '是否拨打客服电话',
