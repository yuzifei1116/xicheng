	...
	<view class="container">
		...
		<view class="time aces-space-between">
			<view class="price-box">
				<view class="acea-row">
					<text class="price-tip">定金：¥</text>
					<text class="dprice">{{storeInfo.price}}</text>
				</view>
				<text class="price-tip">预售价：¥</text>
				<text class="price">{{storeInfo.pre_price}}</text>
			</view>
			<view class="time-s" v-if="isClickBtn">
				<view class="time-tit">预售</view>
				{{when_sold}} 
			</view>
		...
		<view class="introduce-section">
			...
			<view class="yst">
				<!-- <view>定金￥{{storeInfo.price}}，可抵用1</view> -->
				<view class="ysts">支付尾款：{{storeInfo.wk_start_time}}至{{storeInfo.wk_stop_time}}</view>
				<view>温馨提示：过期未付尾款不退定金</view>
			</view>
		</view>
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group">
				<text v-if="isClickBtn">
					<button type="primary" class=" action-btn no-border buy-now-btn" :data-id="2" @click="toggleSpec">支付定金</button>
				</text>
				<text v-else>
...
<script>
	...
	export default{
		...
		methods:{
			...
			details(){
				let self = this;
				let path = '/gzh/presale_api/presale_detail?id='+self.detid
