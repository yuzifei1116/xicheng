<template>
	<view class="content">
		...
		<!-- <swiper class="swiper" :indicator-dots="true" style="height:1200upx;"  previous-margin="60upx" next-margin="60upx"> -->
		    <!-- <swiper-item v-for="(item,index) in beijing" :key="index"> -->
				<!-- {{item}} -->
				<view class="flex_row_c_c modalView" :class="qrShow?'show':''">
					<!-- <view class="flex_row_c_c modalView" :class="qrShow?'show':''"  > -->
					<view class="flex_column">
						<view class="backgroundColor-white padding1vh border_radius_10px">
							<img :src="poster.finalPath || ''" mode="widthFix" class="posterImage">
							<!-- <img :src="item.pic || ''" mode="widthFix" class="posterImage"> -->
						</view>
		...
		methods: {
			my(){
				...
				getApp().http(path, {}, function(res) {
					...
					self.val = self.webUrl+'/gzh/?spuid='+res.uid
					self.haiB();
				})
			},
			haiB(){
				...
				uni.request({
				    url: self.webUrl+'/gzh/auth_api/get_spread_poster',
				    data: {},
				    header: {
				    ...
				    success: (res) => {
				        ...
				        self.beijing = res.data.data;
						console.log("获取 ", self.beijing);
						self.shareFc()
				    }
				});
			},
			async shareFc() {
				...
				try {
					...
					const d = await getSharePoster({
						...
						backgroundImage:self.beijing[0].pic,
						drawArray: ({
							bgObj,
							type,
							bgScale
